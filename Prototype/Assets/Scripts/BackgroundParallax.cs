...
public class BackgroundParallax : MonoBehaviour
{
	...
	private Vector3[] startPos;				// The postion of the camera in the previous frame.
	...
	void Awake ()
	{
		...
		{
			startPos[i] = new Vector3(backgrounds[i].transform.position.x, backgrounds[i].transform.position.y, backgrounds[i].transform.position.z);
		}
	}
	...
	void Update ()
	{
		float parallax = (cam.position.x - transform.position.x) * parallaxScale;

		for(int i = 0; i < backgrounds.Length; i++)
		{
			// ... set a target x position which is their current position plus the parallax multiplied by the reduction.
			float backgroundTargetPosX = startPos[i].x + parallax * (i * parallaxReductionFactor + 1);
